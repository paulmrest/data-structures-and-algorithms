...
const templatingWithMustache = () => {
  let characterTemplates = new Array();
  characters.forEach(oneCharacter => {
    let scriptTemplate = $('#template').html();
    characterTemplates.push(Mustache.render(scriptTemplate, oneCharacter));
  });
...
const getHouses = (arr) => {
  let houses = [];
  arr.forEach(oneCharacter => {
    houses.push(oneCharacter.house);
  });
  return houses;
};
...
const hasChildrenValues = (arr, character) => {
  ...
  arr.forEach(oneCharacter => {
    if (oneCharacter.name === character)
    {
      hasChildren = Object.values(oneCharacter)[2].length > 0 ? true : false;
    }
  });
  return hasChildren;
};
...
const hasChildrenEntries = (arr, character) => {
  ...
  arr.forEach(oneCharacter => {
    if (oneCharacter.name === character)
    {
      hasChildren = Object.entries(oneCharacter)[2][1].length > 0 ? true : false;
    }
  });
  return hasChildren;
};
...
const totalCharacters = (arr) => {
  ...
  arr.forEach(oneCharacter => {
    ...
    {
      totalCharacters++;
      if (oneCharacter.spouse)
      {
        totalCharacters++;
      }
      oneCharacter.children.forEach(() => {
        totalCharacters++;
      })
    }
  });
...
const houseSize = (arr) => {
  const sizes = [];
  arr.forEach(oneCharacter => {
    if (oneCharacter.name && oneCharacter.house)
    {
      let houseExists = false;
      let houseMembersForThisCharacter = 1 + (oneCharacter.spouse ? 1 : 0);
      houseMembersForThisCharacter += oneCharacter.children.length;
      sizes.forEach(oneHouse => {
        if (oneHouse.house === oneCharacter.house)
        {
          houseExists = true;
          oneHouse.members += houseMembersForThisCharacter;
        }
      });
      if (!houseExists)
      {
        sizes.push(
          {
            ...
            members: houseMembersForThisCharacter
          }
        );
      }
    }
...
const houseSurvivors = (arr) => {
  ...
  arr.forEach(oneCharacter => {
    ...
    {
      let houseExists = false;
      let houseMembersForThisCharacter = 1 + (oneCharacter.spouse && !deceasedSpouses.includes(oneCharacter.spouse) ? 1 : 0);
      houseMembersForThisCharacter += oneCharacter.children.length;
      survivors.forEach(oneHouse => {
        if (oneHouse.house === oneCharacter.house)
        {
          houseExists = true;
          oneHouse.members += houseMembersForThisCharacter;

...
const oddValues = (arr) => {
  return arr.filter(oneNum => oneNum % 2 !== 0);
};
...
const filterStringsWithVowels = (arr) => {
  const regex = /[aeiuo]/i;
  return arr.filter(oneWord => regex.test(oneWord));
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  return arr.filter(oneElement => !forbiddenValues.includes(oneElement));
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  return arr.filter(oneStat => {
    return oneStat.baseStat > minBaseStat;
  });
};
...
const getCharactersWithoutChildren = (arr) => {
  return arr.filter(oneCharacter => {
    return !oneCharacter.children;
  });
};
...
const evenOddNumericValues = (arr) => {
  return arr
    .filter(oneValue =>
    {
      return typeof(oneValue) === 'number';
    })
    .map(oneNum =>
    {
      return oneNum % 2 === 0 ? 'even' : 'odd';
    });
};

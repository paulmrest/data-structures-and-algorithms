...
const isCapitalized = (str) => {
  ...
  //testing expects an empty array when there are no matches, but String.match() returns
  //null when there is no match
  const returnArray = str.match(regex);
  return returnArray ? returnArray : new Array();
};
...
const citiesAtoJ = (arr) => {
  const aAndJCities = new Array();
  const regex = /^[A-J]/;
  arr.forEach(oneCity => {
    if (regex.test(oneCity))
    {
      aAndJCities.push(oneCity);
    }
  });
  return aAndJCities;
};
...
const matchMonth = (input) => {
  const regex1 = /\b[Oo]ctober\b/;
  const regex2 = /\b[Oo]ct\b/;

...
const forLoopTwoToThe = (arr) => {
  let twoToThePower = new Array();
  for (let oneNum of arr)
  {
    twoToThePower.push(Math.pow(2, oneNum));
  }
...
const extractAbilities = (arr) => {
  return arr.map(oneAbility => {
    return oneAbility.ability.name;
  })
};
...
const extractStats = (arr) => {
  return arr.map(oneStat => {
    return {
      name: oneStat.stat.name,
      total: oneStat.effort + oneStat.baseStat
    };
  })
};

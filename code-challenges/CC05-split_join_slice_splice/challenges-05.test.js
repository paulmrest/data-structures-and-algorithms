...
const templateWithJQuery = () => {
  ...
  starWarsPeople.forEach(onePerson => {
...
const howMuchPencil = (str) => {
  ...
  {
    workingStr = workingStr.slice(1, workingStr.length);
    result.push(workingStr);
  }
  return result;
};
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(oneIngredient => {
    let toFirstSpace = oneIngredient.slice(oneIngredient.indexOf(' ') + 1);
    let toSecondSpace = toFirstSpace.slice(toFirstSpace.indexOf(' ') + 1);
    result.push(toSecondSpace);
  });
  return result;
};
...
const splitFoods = (recipe) => {
  let result = [];
  recipe.ingredients.forEach(oneIngredient => {
    let spaceSplitArray = oneIngredient.split(' ');
    result.push(spaceSplitArray.slice(2).join(' '));
  });
  return result;
};
...
const stepActions = (recipe) => {
  let result = [];
  recipe.steps.forEach(oneStep => {
    result.push(oneStep.slice(0, oneStep.indexOf(' ')));
  });
  return result;
};
...
const totalSumCSV = (str) => {
  let total = 0;
  str.split(',').forEach(numberAsString => {
    total += Number(numberAsString);
  });
  return total;
};
...
const extractVowels = (str) => {
  ...
  let vowelsRegex = /[aeiou]/gi;
  returnArray.push(str.replace(vowelsRegex, ''));
  returnArray.push(str.match(vowelsRegex).sort().join(''));
  return returnArray;
};

...
const toLastNames = people => {
  return people.map(onePerson => {
    return `${onePerson.firstName} ${onePerson.lastName}`;
  })
};
...
const validateEmail = (email) => {
  const regex = /^[A-Z0-9]{1,}.?[A-Z0-9]{1,}@[A-Z0-9]{1,}\.[A-Z0-9]{3}$/i;
...
const validatePhoneNumber = (phoneNumber) => {
  const regex = /^\d{10}$|^\d{6}-\d{4}$|^\d{3}\s\d{3}\s\d{4}$|^\d{3}-\d{3}-\d{4}$|^\d{3}-\d{3}\s\d{4}$|^\d{3}-\d{7}$|^\d{3}\s\d{3}-\d{4}$|^\(\d{3}\)\d{3}\s\d{4}$|^\(\d{3}\)\s\d{3}-\d{4}$|^\d{3}\s\d{7}$/gi;

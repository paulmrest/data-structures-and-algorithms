...
const mapCurrentEvents = () => {
  const eventObjects = currentEvents.news.map(oneEvent => {
    return new Event(oneEvent);
  });
  return eventObjects;
}
...
const countNumberOfElements = (arr) => {
  const numOfElements = arr.reduce((accumulator, oneElement) => {
...
const returnNames = (arr) => {
  const namesArray = starWarsData.reduce((accArray, oneElement) => {
    accArray.push(oneElement.name);
    return accArray;
  }, new Array());
  return namesArray;
};
...
const reversedString = (str) => {
  const revStr = str.split('').reduce((accString, oneChar) => {
    accString = oneChar + accString;
    return accString;
  }, '');
  return revStr;
};
...
const countNumberOfChildren = (arr) => {
  const totalChildren = characters.reduce((accChildren, oneCharacter) => {
    if (oneCharacter.children)
    {
      accChildren += oneCharacter.children.length;
    }
    return accChildren;
  }, 0);
...
const calculateAverage = (arr) => {
  const countAndSum = arr.reduce((averageAcc, oneNum) => {
    return {
      count: averageAcc.count + 1,
      sum: averageAcc.sum + oneNum
    };
  }, {count: 0, sum: 0});
...
const countPrimeNumbers = (arr) => {
  return arr.reduce((primeCount, currNum) => {
    primeCount += isPrime(currNum) ? 1 : 0;
    return primeCount;
  }, 0);
};
...
const extractStat = (statName, arr) => {
  return arr.reduce((accRtnObject, currObject) => {
    if (currObject.stat.name === statName)
    {
      accRtnObject = currObject;
    }
    return accRtnObject;
  }, null);
};
...
const extractChildren = (arr) => {
  return arr
    .filter(oneCharacter => {
      return oneCharacter.name.toLowerCase().includes('a');
    })
    .reduce((childrenAcc, oneACharacter) => {
      if (oneACharacter.children)
      {
        childrenAcc = childrenAcc.concat(oneACharacter.children);
      }
      return childrenAcc;
    }, new Array());
};

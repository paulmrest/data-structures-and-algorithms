...
const createServer = () => {
  ...
  app.get('/hello', (request, response) => {
    response.status(200).send('Well hello out there!');
  });

  app.get('/aboutme', (request, response) => {
    response.status(200).send('Still don\'t have rocket legs...');
  });
  ...
  // console.log('About to return the server...whole server:');
  // console.log(server);
  // console.log('server._events.get');
  // console.log(server._events.get);
...
const count = (target, input) => {
  return input.reduce((acc, oneInnerArray) => {
    acc += oneInnerArray.filter(oneNum => {
      return oneNum === target;
    }).length;
    return acc;
  }, 0);
};
...
const totalSum = (input) => {
  return input
    .reduce((innerSumAcc, oneInnerArray) => {
      const innerSum = oneInnerArray.reduce((sumAcc, oneNum) => {
        sumAcc += oneNum;
        return sumAcc;
      }, 0);
      innerSumAcc.push(innerSum);
      return innerSumAcc;
    }, new Array())
    .reduce((outerSumAcc, oneInnerSum) => {
      outerSumAcc += oneInnerSum;
      return outerSumAcc;
    }, 0)
};
...
const divisibleByFiveTwoToThePower = (input) => {
  let returnArray = input.map(oneInnerArray => {
    let filteredInnerArray = oneInnerArray.filter(oneElement => {
      return typeof(oneElement) === 'number' && oneElement % 5 === 0;
    });
    return filteredInnerArray.map(oneNum => {
      return Math.pow(2, oneNum);
    });
  });
  return returnArray;
};
...
let findMaleAndFemale = (data) => {
  return data.filter(oneCharacter => {
    
  })
  .reduce((nameStringAcc, oneCharacter, index) => {
...
describe('Testing challenge 1', () => {
  ...
  test('responds to /hello', function testHello(done) {
    ...
    // console.log(request);
    // console.log(request(server).get);
    // console.log(server);
